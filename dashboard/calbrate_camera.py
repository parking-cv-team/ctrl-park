...
# ---------------------------------------------------------------------------
def save_topdown(cam_name: str, rows: int, cols: int,
...
# Config file I/O
# ---------------------------------------------------------------------------
def load_config(cam_name: str) -> dict | None:
    try:
        response = requests.get(f"{API_BASE}/camera/config",params={"cam_name":cam_name})
...
def save_config_file(params: dict, ctrl_pts: list,cam_name):
...
def run_zone_creator(uri: str,cam_name: str):
    ...
    # ------------------------------------------------------------------
    initial_params = load_config(cam_name)
    ...
    if frame is None:
        st.error(f"[calibration]: Cannot read frame {frame_idx_init} from {uri}")
        return None
    ...
    if H is None:
        #st.error(f"[calibration]: exiting without saving")
        return None
    ...
    if selector.cancelled:
        st.error("[calibrate] Exiting without saving.")
        return None
