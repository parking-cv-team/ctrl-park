...
def _grab_frame(uri: str) -> np.ndarray: # should be good
...
def _load_cameras(): # good
    try:
        cams = requests.get(f"{API_BASE}/analytics/cameras")
        cams = cams.json()
        return [{"id": c["id"], "name": c["name"], "uri": c["uri"]} for c in cams]
